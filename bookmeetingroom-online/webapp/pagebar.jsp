...
<%
	out.write("<div class='pager'>");
		out.write("<span class='showtext'>约" + resultPage.getCount() + "条，分"
				+ resultPage.getPageCount() + "页</span>");
		...
		if (resultPage.getPage() > 1) {
			out
					.write(" <span class='btn' style='cursor: pointer;' onclick='javascript:goPage("
							+ (resultPage.getPage() - 1) + "); '>◀上一页</span>&nbsp;");
		}
		if (resultPage.getPage() == 1&&resultPage.getPageCount()!=1) {
			out.write(" <span  style='color: #CCCCCC;'>◀上一页</span>&nbsp;");
		}
		...
		if (resultPage.getPageCount() > (resultPage.getPageSize() + pp)) {
			out.write("<a href='javascript:goPage(" + resultPage.getPageCount()
					+ ")'><font color=red>" + resultPage.getPageCount()
					+ "</font></a>&nbsp;");
		}
		if (resultPage.getPage() < resultPage.getPageCount()) {
			out
					.write("<span class='btn' style='cursor: pointer;'  onclick='javascript:goPage("
							+ (resultPage.getPage() + 1) + "); '>下一页▶</span></div>");
		}
		if (resultPage.getPage() == resultPage.getPageCount()&& resultPage.getPage()>1) {
			out.write("<span style='color: #CCCCCC;'>下一页▶</span></div>");
		}
		out.write("</div>");
	}
%>